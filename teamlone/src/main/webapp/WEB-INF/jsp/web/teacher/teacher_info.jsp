				...
				<section class="fl t-infor-box c-desc-content">
					<div class="mt20 ml20">
						<section class="t-infor-pic">
							<c:choose>
								<c:when test="${not empty th.pic_path }">
									<img src="${ctx }/static/inxweb/img/default-tea-img.gif" xsrc="<%=staticImage %>${th.pic_path}" alt="">
								</c:when>
								<c:otherwise>
						...
						<h3 class="hLh30">
							<span class="fsize24 c-333">${th.name } <c:if test="${th.th_name =='高级讲师'}">&nbsp;高级讲师</c:if> <c:if test="${th.th_name=='首席讲师'}">&nbsp;首席讲师</c:if></span>
						</h3>
						<section class="mt10">
							<span class="t-tag-bg">${th.education }</span>
						</section>
						<section class="t-infor-txt">
							<p class="mt20">${th.career }</p>
						</section>
						<div class="clear"></div>
					</div>
				</section>
