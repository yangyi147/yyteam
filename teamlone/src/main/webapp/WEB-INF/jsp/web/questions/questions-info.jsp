					...
					<section class="mr30 pt10">
						<section class="path-wrap txtOf hLh30">
							<a href="${ctx }" title="" class="c-999 fsize14">首页</a> \ <a
							 ...
							 <span class="c-333 fsize14">${qt.title }</span>
						</section>
						<!-- /问题详情 开始 -->
						<div>
							<section class="q-infor-box">
								<div class="pr">
									<aside class="q-head-pic">
										<c:choose>
											<c:when test="${not empty qt.edu_user.pic_img }">
												<img src="<%=staticImage %>${qt.edu_user.pic_img }" alt="">
											</c:when>
											<c:otherwise>
										...
										<h3 class="hLh30 txtOf">
											<em class="icon16 q-tw">&nbsp;</em> <span
												class="c-blue fsize14"> <c:if
													test="${empty qt.edu_user.show_name }">${qt.edu_user.email }</c:if>
												<c:if test="${not empty qt.edu_user.show_name }">${qt.edu_user.show_name }</c:if>
											</span> <span class="c-999 fsize14"> <c:if
													test="${qt.type==1 }">课程提问</c:if> <c:if
													test="${qt.type==2 }">学习分享</c:if>
											</span>
										</h3>
									</section>
									<section class="ml50 pl10">
										<div class="mt20">
											<h3 class="hLh30 txtOf">
												<span class="fsize18 c-333 vam">${qt.title }</span>
											</h3>
										</div>
										<div class="i-q-txt mt15">

											<span class="c-999 f-fA"><c:out
													value="${qt.content}"></c:out></span>
											...
											<section class="fr">
												<span> <a href="#i-art-comment" title="评论"
													class="noter-dy vam"> <em class="icon18">&nbsp;</em>(<span
														id="questionsReplyCount">${qt.reply_count }</span>)
												</a> <tt class="noter-zan vam ml10 f-fM" title="赞一下"
														onclick="addPraise(${qt.id },1,this)">
														<em class="icon18">&nbsp;</em>(<span>${qt.praise_count }</span>)
													</tt>
												</span>
											</section>
											<span class="c-ccc fl vam"><fmt:formatDate
																value="${qt.add_time }" type="date"
							...
							<div class="taglist clearfix">
								<form action="${ctx}/questions/list" id="searchForm"
									...
									<input type="hidden" id="pageCurrentPage"
										...
										name="questions.orderFalg" value="${qt.add_time}" />
									<input type="hidden" name="questions.type"
										value="${qt.type}" /> <input type="hidden"
										name="questions.status" value="${qt.status}" /> <input
